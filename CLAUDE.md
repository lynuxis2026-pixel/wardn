...
1. **Discover** — lees bestaande client-configs (Claude Desktop, Cursor, VS Code), lijst alle servers. ✅
2. **Scan** — trust-score per server via uitlegbare heuristiek. ✅
3. **Visualize** — dashboard met servers, permissies, trust-score. ⏳ (CLI-versie ✅)
...
- [ ] **Sandbox-flow** — sandboxen verandert aantoonbaar wat een server kan.
- [ ] **Proxy-flow** — echte tool-call loopt via de gateway, identiek, zichtbaar in de live-log.
- [ ] **Cross-client-flow** — config-rewrite werkt in alle clients én is terug te draaien.
...
1. Elk nieuw project start met een CLAUDE.md + memory-file. ✅
